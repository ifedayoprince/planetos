; ==================================================================
; PlanetOS -- The Planet Operating System kernel
; Copyright (C) 2006 - 2020 PlanetOS Developers -- see doc/LICENSE.TXT
;
...
get_cmd:
	...
	mov di, boot_string		; 'REBOOT' entered?
	call os_string_compare
	jc near reboot
	
	mov di, about_string		; 'DEV' entered?
	call os_string_compare
	jc near about
...
bin_fail:
	...
	mov byte [si], '.'		; See if there's a .SC extension for this
	mov byte [si+1], 'S'
	mov byte [si+2], 'C'
	mov byte [si+3], 0

	mov ax, input			; Try to load the .SC code
	mov bx, 0
	mov cx, 32768
	call os_load_file
	jc total_fail			; Skip if program not found
...
about:
	...
	mov si,name							; Prints all the details about the OS
	call os_print_string
	call os_print_newline

	mov si,copy							; Prints the copyright authority
	call os_print_string
	call os_print_newline

	mov si,dev							; Prints information about the Devs
	call os_print_string
	call os_print_newline

	mov si,prom							; Prints the dev's promise
	call os_print_string
	call os_print_newline
	
	name db 'PlanetOS ',PLANETOS_VER, ' Release Date: ', PLANETOS_RELEASED,0
	copy db 'Copyright(c) 2020 InterNerds Inc. ',PLANETOS_VER,0
	dev db 'The PlanetOS Team',0
	prom db 'We promise to keep on giving you the best. With new bug fixes and improvements',0
...
; ------------------------------------------------------------------
	...
	prompt			db '> ', 0
	help_text		db 'DIR - Displays all the installed apps in PlanetOS;CAT - Displays the background code running behind the apps;CLS - Clears the screen; HELP - Displays the list of commands; TIME - Displays the current time; DATE - Displays the current date; VER - Displays the OS version; EXIT - Goes back to GUI Mode; REBOOT - Reboots PlanetOS; ABOUT - Display everything you need to know about PlanetOS', 13, 10, 0
	...
	version_msg		db 'PlanetOS ', PLANETOS_VER, 13, 10, 0
	...
	ver_string		db 'VER', 0
	cat_string		db 'CAT', 0
	boot_string		db 'REBOOT', 0
	about_string		db 'ABOUT', 0

	kern_file_string	db 'KERNEL.BIN', 0
	kern_warn_msg		db 'This is PlanetOS System File and cannot be executed!', 13, 10, 0
