...
file_chosen:
	...
	mov si, sc_extension			; Check for .BAS extension
	mov cx, 3
	rep cmpsb
	je valid_extension
...
.f5_pressed:				; Run SC
	...
	.sc_finished_msg	db ">>> SC finished - hit a key to return to the editor", 0
...
; Data section
	...
	txt_title_msg	db 'PlanetOS Editor', 0
	txt_footer_msg	db '[Esc] Quit    [F1] Help    [F2] Save    [F5] Run SC    [F8] Run BASIC', 0

	txt_extension	db 'TXT', 0
	bas_extension	db 'BAS', 0
	sc_extension	db 'SC',0
	wrong_ext_msg	db 'You can only load .TXT, .SC or .BAS files!', 0
